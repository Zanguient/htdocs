"use strict";function initAngularDataTables(){$.fn.DataTable.Api&&$.fn.DataTable.Api.register("ngDestroy()",function(a){return a=a||!1,this.iterator("table",function(e){var t,n=e.nTableWrapper.parentNode,s=e.oClasses,r=e.nTable,i=e.nTBody,o=e.nTHead,l=e.nTFoot,d=$(r),b=$(i),c=$(e.nTableWrapper),p=$.map(e.aoData,function(a){return a.nTr});e.bDestroying=!0,$.fn.DataTable.ext.internal._fnCallbackFire(e,"aoDestroyCallback","destroy",[e]),a||new $.fn.DataTable.Api(e).columns().visible(!0),c.unbind(".DT").find(":not(tbody *)").unbind(".DT"),$(window).unbind(".DT-"+e.sInstance),r!==o.parentNode&&(d.children("thead").detach(),d.append(o)),l&&r!==l.parentNode&&(d.children("tfoot").detach(),d.append(l)),d.detach(),c.detach(),e.aaSorting=[],e.aaSortingFixed=[],$.fn.DataTable.ext.internal._fnSortingClasses(e),$(p).removeClass(e.asStripeClasses.join(" ")),$("th, td",o).removeClass(s.sSortable+" "+s.sSortableAsc+" "+s.sSortableDesc+" "+s.sSortableNone),e.bJUI&&($("th span."+s.sSortIcon+", td span."+s.sSortIcon,o).detach(),$("th, td",o).each(function(){var a=$("div."+s.sSortJUIWrapper,this);$(this).append(a.contents()),a.detach()})),!a&&n&&(n.contains(e.nTableReinsertBefore)?n.insertBefore(r,e.nTableReinsertBefore):n.appendChild(r)),d.css("width",e.sDestroyWidth).removeClass(s.sTable),(t=e.asDestroyStripes.length)&&b.children().each(function(a){$(this).addClass(e.asDestroyStripes[a%t])});var h=$.inArray(e,$.fn.DataTable.settings);-1!==h&&$.fn.DataTable.settings.splice(h,1)})})}angular.module("datatables",["datatables.directive","datatables.factory"]).run(initAngularDataTables);

"use strict";function dtInstanceFactory(){function a(a){var e=Object.create(c);return e._renderer=a,e}function e(a,e){e.id=a.id,e.DataTable=a.DataTable,e.dataTable=a.dataTable}function t(a,e){this._renderer.reloadData(a,e)}function r(a){this._renderer.changeData(a)}function n(){this._renderer.rerender()}var c={reloadData:t,changeData:r,rerender:n};return{newDTInstance:a,copyDTProperties:e}}angular.module("datatables.instances",["datatables.util"]).factory("DTInstanceFactory",dtInstanceFactory);

"use strict";function dtDefaultOptions(){function t(t){return l.loadingTemplate=t,l}function a(t){return $.ajax({dataType:"json",url:t,success:function(t){$.extend(!0,$.fn.DataTable.defaults,{language:t})}}),l}function n(t){return $.extend(!0,$.fn.DataTable.defaults,{language:t}),l}function e(t){return $.extend($.fn.DataTable.defaults,{displayLength:t}),l}function o(t){return l.bootstrapOptions=t,l}function s(t){return $.extend($.fn.DataTable.defaults,{dom:t}),l}function u(t,a){if(angular.isString(t)){var n={};n[t]=a,$.extend($.fn.DataTable.defaults,n)}}var l={loadingTemplate:"<h3>Carregando...</h3>",bootstrapOptions:{},setLoadingTemplate:t,setLanguageSource:a,setLanguage:n,setDisplayLength:e,setBootstrapOptions:o,setDOM:s,setOption:u};return l}angular.module("datatables.options",[]).constant("DT_DEFAULT_OPTIONS",{sAjaxDataProp:"",aoColumns:[]}).constant("DT_LOADING_CLASS","dt-loading").service("DTDefaultOptions",dtDefaultOptions);

"use strict";angular.module('datatables.util', []).factory('DTPropertyUtil', x);function x($q){return {overrideProperties: a,deleteProperty: b,resolveObjectPromises: c,resolveArrayPromises: d};function a(r,e){var n=angular.copy(r);if((angular.isUndefined(n)||null===n)&&(n={}),angular.isUndefined(e)||null===e)return n;if(angular.isObject(e))for(var a in e)e.hasOwnProperty(a)&&(n[a]=overrideProperties(n[a],e[a]));else n=angular.copy(e);return n}function b(e,t){angular.isObject(e)&&delete e[t]}function c(r,e){var a=$q.defer(),n=[],s={},i=e||[];if(!angular.isObject(r)||angular.isArray(r))a.resolve(r);else{s=angular.extend(s,r);for(var o in s)s.hasOwnProperty(o)&&-1===$.inArray(o,i)&&(angular.isArray(s[o])?n.push(resolveArrayPromises(s[o])):n.push($q.when(s[o])));$q.all(n).then(function(r){var e=0;for(var n in s)s.hasOwnProperty(n)&&-1===$.inArray(n,i)&&(s[n]=r[e++]);a.resolve(s)})}return a.promise}function d(r){var e=$q.defer(),n=[],a=[];return angular.isArray(r)?(angular.forEach(r,function(r){angular.isObject(r)?n.push(resolveObjectPromises(r)):n.push($q.when(r))}),$q.all(n).then(function(r){angular.forEach(r,function(r){a.push(r)}),e.resolve(a)})):e.resolve(r),e.promise}}

"use strict";angular.module('datatables.directive', ['datatables.instances', 'datatables.renderer', 'datatables.options', 'datatables.util']).directive('datatable', dataTable);function dataTable($q, $http, $log, DTRendererFactory, DTRendererService, DTPropertyUtil) {return {restrict: 'A',scope: {dtOptions: '=',dtColumns: '=',dtColumnDefs: '=',datatable: '@',dtInstance: '='},compile: cd,controller: ct};function cd(e){var n=e[0].innerHTML;return function(e,i,r,o){function t(e,r){e!==r&&o.render(i,o.buildOptionsPromise(),n)}var c=r.dtDisableDeepWatchers?"$watchCollection":"$watch";angular.forEach(["dtColumns","dtColumnDefs","dtOptions"],function(n){e[c].call(e,n,t,!0)}),DTRendererService.showLoading(i,e),o.render(i,o.buildOptionsPromise(),n)}}function ct($scope) {var _dtInstance;var vm = this;vm.buildOptionsPromise = a;vm.render = b;function a(){var e=$q.defer();return $q.all([$q.when($scope.dtOptions),$q.when($scope.dtColumns),$q.when($scope.dtColumnDefs)]).then(function(e){var r=e[0],t=e[1],o=e[2];DTPropertyUtil.deleteProperty(r,"$promise"),DTPropertyUtil.deleteProperty(t,"$promise"),DTPropertyUtil.deleteProperty(o,"$promise");var n;if(angular.isDefined(r)&&(n={},angular.extend(n,r),angular.isArray(t)&&(n.aoColumns=t),angular.isArray(o)&&(n.aoColumnDefs=o),n.language&&n.language.url)){var a=$q.defer(),l=n.language.url;$http.get(n.language.url).then(function(e){a.resolve(e.data)},function(){$log.error("Could not fetch the content of the language from "+l)}),n.language=a.promise}return DTPropertyUtil.resolveObjectPromises(n,["data","aaData","fnPromise"])}).then(function(r){e.resolve(r)}),e.promise}function b(e,n,t){n.then(function(n){DTRendererService.preRender(n);var r=$scope.datatable&&"ng"===$scope.datatable;_dtInstance&&_dtInstance._renderer?_dtInstance._renderer.withOptions(n).render(e,$scope,t).then(function(e){_dtInstance=e,c(e)}):DTRendererFactory.fromOptions(n,r).render(e,$scope,t).then(function(e){_dtInstance=e,c(e)})})}function c(n){angular.isFunction($scope.dtInstance)?$scope.dtInstance(n):angular.isDefined($scope.dtInstance)&&($scope.dtInstance=n)}}}
...
function showLoading(e,a){var n=angular.element(DTLoadingTemplate.compileHtml(a));e.after(n),$(e).hide(),$(n).show()}function hideLoading(e){$(e).show();var a=e.next();DTLoadingTemplate.isLoading(a)&&a.remove()}function renderDataTable(e,a){var n="#"+e.attr("id");$.fn.dataTable.isDataTable(n)&&angular.isObject(a)&&(a.destroy=!0);var r=$(e).DataTable(a),t=$(e).dataTable(),i={id:e.attr("id"),DataTable:r,dataTable:t};return postRender(a,i),i}function hideLoadingAndRenderDataTable(e,a){return rendererService.hideLoading(e),rendererService.renderDataTable(e,a)}function registerPlugin(e){plugins.push(e)}function postRender(e,a){angular.forEach(plugins,function(n){angular.isFunction(n.postRender)&&n.postRender(e,a)})}function preRender(e){angular.forEach(plugins,function(a){angular.isFunction(a.preRender)&&a.preRender(e)})}
}

function dtRenderer(){return{withOptions:function(t){return this.options=t,this}}}
...
function render(e,n){_$elem=e,_$scope=n;var r=DTInstanceFactory.newDTInstance(renderer),a=DTRendererService.hideLoadingAndRenderDataTable(e,renderer.options);return _oTable=a.DataTable,DTInstanceFactory.copyDTProperties(a,r),$q.when(r)}
function reloadData(){DTRendererService.showLoading(_$elem,_$scope)}
function changeData(){DTRendererService.showLoading(_$elem,_$scope)}
