...
echo Limpando estrutura de %TelaID%/%Grupos% - %TelaNO%/%SGrupos%

cd..
@rem ######################################-Controller-################################
echo Controller...
IF EXIST "app\Http\Controllers\%Grupos%" (
	del .\app\Http\Controllers\%Grupos%\%TelaNO%Controller.php   /s /q
)
@rem #################################################################################
...
IF EXIST "app\Http\Routes\%Grupos%" (
	del .\app\Http\Routes\%Grupos%\%TelaNO%.php  /s /q
)
@rem #################################################################################
...
IF EXIST "app\Models\DAO\%Grupos%" (
	del .\app\Models\DAO\%Grupos%\%TelaNO%DAO.php  /s /q
)
@rem #################################################################################
...
IF EXIST "app\Models\DTO\%Grupos%" (
	del .\app\Models\DTO\%Grupos%\%TelaNO%.php  /s /q
)
@rem #################################################################################

@rem ########################################-JS-#####################################
echo JS...
IF EXIST "resources\assets\js\%SGrupos%\%TelaNO%" (
	rd .\resources\assets\js\%SGrupos%\%TelaNO%  /s /q
)
@rem #################################################################################
...
IF EXIST "resources\lang\pt-BR\%SGrupos%" (
	del .\resources\lang\pt-BR\%SGrupos%\%TelaNO%.php  /s /q
)
@rem #################################################################################

@rem ########################################-views-##################################
echo views...
IF EXIST "resources\views\%SGrupos%\%TelaNO%" (
	rd .\resources\views\%SGrupos%\%TelaNO%  /s /q
)
@rem #################################################################################
