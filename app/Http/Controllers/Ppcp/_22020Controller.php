...
class _22020Controller extends Controller
{
    /**
    ...
    {   
//        if ( env('REGISTRO_PRODUCAO',0) == 1 && sizeof(userMenu('ppcp/_22010',0,null,false)) > 0 ) {
//            
//            return redirect('/_22010');
//        } else {

            $permissaoMenu	= _11010::permissaoMenu($this->menu);
            $ps227			= _11000::controle(227);
...
//        }
    ...
    public function show(Request $request,$id)
    {
        ...
        if ( strripos($request->url(), 'show') ) {       
            $view = 'ppcp._22020.show.body';
        } else {
            $view = 'ppcp._22020.show';
        }
    ...
    public function talaoTotalizador(Request $request)
    {
        ...
        if ( !empty($request->retorno) && in_array('VIEW', $request->retorno) ) {
            $ret = view('ppcp._22020.index.talao-produzir',[
                'menu'   => $this->menu,
     ...
     * @param Request $request
     */
    public function talaoProduzir(Request $request)
    {
		$param = (object)[
            'RETORNO'            => ['TALAO'],
            'STATUS'             => [1] // Status 1 = Em Aberto
    ...
    public function talaoProduzido(Request $request)
    {
        ...
        if ( !empty($request->retorno) && in_array('VIEW', $request->retorno) ) {
            $ret = view('ppcp._22020.index.talao-produzido',[
                'menu'					=> $this->menu,
    ...
    public function talaoDetalhe(Request $request)
    {
        ...
        if ( !empty($request->retorno) && in_array('VIEW', $request->retorno) ) {
			...
			$ret = View::make('ppcp._22020.index.talao-detalhe')
						->with('menu', $this->menu)
    ...
    public function talaoHistorico(Request $request)
    {
        ...
        if ( !empty($request->retorno) && in_array('VIEW', $request->retorno) ) {
            ...
            $ret = View::make('ppcp._22020.index.talao-historico')
						->with('menu', $this->menu)
    ...
    public function alterarQtdSobraMaterial(Request $request){
        return _22020::alterarQtdSobraMaterial(['CONSUMO_ID' => $request->consumo_id, 'SOBRA' => $request->sobra]);
    }
    ...
    {
        $taloes_materia_prima = orderBy(_22020::materiaPrima([
        ...
        $taloes_vinculo = _22020::projecaoVinculo([
        ...
        if ( !empty($request->retorno) && in_array('VIEW', $request->retorno) ) {
            ...
            $ret = 
            View::make('ppcp._22020.index.talao-materia'				)
                ->with('menu'					, $this->menu			)
		...
		if ( $ver_peca_disponivel === '1' ) {
			...
			foreach ($taloes_materia_prima as $t) {
				...
				if( trim($t->COMPONENTE) == '0') {
					...
					array_push(
						$pecas_disponiveis, 
						(object)_22020::consultarPecaDisponivel([
    ...
    public function talaoDefeito(Request $request)
    {
        ...
        if ( !empty($request->retorno) && in_array('VIEW', $request->retorno) ) {
            ...
            $ret = View::make('ppcp._22020.index.talao-defeito')
						->with('menu', $this->menu)
...
//            'ESTACAO'            => $request->estacao //Verificação por estação desativada  
	...
	public function verificarEstacaoAtiva(Request $request) {
		...
		$res = _22020::verificarEstacaoAtiva([
			'UP_ID'		 => $request->up_id,
	...
	public function alterarQtdTalaoDetalhe(Request $request) {
		...
		$res = _22020::alterarQtdTalaoDetalhe([
			'RETORNO'			=> $request->retorno,
			'QUANTIDADE'		=> $request->qtd,
            'SOBRA'				=> $request->sbr,
			'TALAO_DETALHE_ID'	=> $request->talao_detalhe_id,
	...
	public function alterarTodasQtdTalaoDetalhe(Request $request) {
		...
		$res = _22020::alterarTodasQtdTalaoDetalhe([
			'REMESSA_ID'		=> $request->REMESSA_ID,
			'REMESSA_TALAO_ID'	=> $request->REMESSA_TALAO_ID,
            'TIPO'              => $request->TIPO
		]);
    ...
    {     
        ...
        $tag = _22020Controller::randomName();
        ...
        $materia_prima = _22020::materiaPrimaSobra([
            'RETORNO'            => ['MATERIA_PRIMA'],
            'TALAO_ID'           => $request->id
        ]);
        
        $itens_materia_prima = _22020::itensmateriaPrima([
            'RETORNO'            => ['MATERIA_PRIMA'],
            'TALAO_ID'           => $request->id
        ]);
        ...
        if ( isset($request->retorno) && in_array('PRODUCAO', $request->retorno) ) {
            ...
            if ( trim($taloes_detalhado[0]->GP_PERFIL) == 'L' || trim($taloes_detalhado[0]->GP_PERFIL) == 'C')
            {
                ...
                if ( isset($request->retorno) && in_array('SOBRA', $request->retorno) ) {
                    ...
                    if($cods == ''){
                        ...
                        $sobra_aproveitamento = _22020::itemTbRevisao([
                            'TABELA_ID'           => $cods
                        ]);
                    } 
                        ...
                        foreach ( $sobra_aproveitamento as $sobra ) {
                            ...
                            if($sobra->SALDO > 0){
                                ...
                                if (isset($talao[0])) {
                                    ...
                                    $str_talao = str_replace('#LINHA6#'  , ''                                           , $str_talao);                    
                                    ...
                                    $str_talao = str_replace('#LINHA6#'  , 'OB:'.$talao->OB                                      , $str_talao);                    
                ...
                foreach ( $materia_prima as $materia ) {
                    ...
                    $str_talao = '';
                        ...
                        if($prima->QUANTIDADE_SOBRA > 0){
                            ...
                            $taloes = _22020::remessaOrigemConsumo(['CONSUMO_ID' => $prima->CONSUMO_ID]);
...
//                                $codigo_barras        = 'RD' . str_pad($talao->ID, 11, "0", STR_PAD_LEFT);
            ...
            {            
                ...
                $str_talao = str_replace('#DATA_HORA_PRODUCAO#' , $datahora_producao         , $str_talao);                          
                $str_talao = str_replace('#OPERADOR#'           , $talao1->OPERADOR_DESCRICAO, $str_talao);                          
                ...
                if ( isset($request->retorno) && in_array('SOBRA', $request->retorno) ) {
                    ...
                    if($cods == ''){
                        ...
                        $sobra_tecido = _22020::itemTbRevisao([
                            'TABELA_ID'           => $cods
                        ]);
                    }
                ...
                if($PERFIL_UP){
                    ...
                    {   
                        ...
                        foreach ($taloes_detalhado as $detalhe){
                            ...
                            $a = str_pad($talao->CONTROLE.'', 4, '0', STR_PAD_LEFT);

                            $pos = substr_count($detalhe->TALOES_ORIGEM . '', $a . '');
                            ...
                            if ($pos == 1) {
                                ...
                                /*
                                log_info($detalhado);
                                log_info($detalhe->TALOES_ORIGEM);
                                log_info($talao->CONTROLE );
                                log_info('');
                                */
                            }
                        }
        ...
        if ( !(trim($string) == '')) {
            _22020::updateTalaoViaEtiqueta($request->id);
	...
	public function remessaOrigemConsumo($consumo_id) {
		...
		$res = _22020::remessaOrigemConsumo(['consumo_id' => $consumo_id->estabelecimento_id]);
    ...
    {       
        ...
        elseif ( strstr($cod_barras, 'P') ){
            ...
            $verif_aproveit = _22020::verificarAproveitamento(['ID' => $id]);
        }
        else {
            log_erro('Código de barras inválido.. COD:'.$cod_barras);
        }
        ...
        if ( !isset($verif_aproveit[0]) ) {
			...
			_22020::registrarAproveitamento($param);
	...
	public function autenticarUp(Request $request) {
		...
		$up_barra_obj = _22020::autenticarUp(['UP_BARRA' => strtoupper($request->up_barra)]);
	...
	public function totalizadorDiario(Request $request) {
        ...
        $res = _22020::totalizadorDiario($param);
//		$conexao = new _Conexao();
//		
//		$tot_programado		= _22020::totalizadorProgramado($conexao, $param);
//		$tot_produzido		= _22020::totalizadorProduzido($conexao, $param);
//		$tot_par_remessa	= _22020::totalizadorParPorDataRemessa($conexao, $param);
//		$tot_par_producao	= _22020::totalizadorParPorDataProducao($conexao, $param);
//		
//		$res = [];		
//		foreach($tot_programado as $i => $tot) {
//			
//			//percentuais
//			$perc_carga_programada	= (($tot->CARGA_PROGRAMADA * 100) / ($tot->CAPACIDADE_DISPONIVEL > 0 ? $tot->CAPACIDADE_DISPONIVEL : 1));
//			$perc_carga_utilizada	= (($tot_produzido[$i]->CARGA_UTILIZADA * 100) / ($tot->CARGA_PROGRAMADA > 0 ? $tot->CARGA_PROGRAMADA : 1));
//			$perc_aproveitamento	= (($tot_produzido[$i]->CARGA_UTILIZADA * 100) / ($tot->CAPACIDADE_DISPONIVEL > 0 ? $tot->CAPACIDADE_DISPONIVEL : 1));
//
//			//eficiencia = ((qtd.prod. / min.prod.) / (qtd.proj. / min.proj.)) * 100
//			$eficiencia				= (((float)($tot_produzido[$i]->QUANTIDADE_CARGA_UTILIZADA / ($tot_produzido[$i]->CARGA_UTILIZADA > 0 ? $tot_produzido[$i]->CARGA_UTILIZADA : 1))
//										/
//										(float)($tot_produzido[$i]->QUANTIDADE_CARGA_PROG_UTILIZ / ($tot_produzido[$i]->CARGA_PROG_UTILIZ > 0 ? $tot_produzido[$i]->CARGA_PROG_UTILIZ : 1)))
//									  * 100); 
//			
//			$arr = (object) [
//				'REMESSA_DATA'					=> $tot->REMESSA_DATA								,
//				'UP_ID'							=> $tot->UP_ID										,
//				'UP_DESCRICAO'					=> $tot->UP_DESCRICAO								,
//				'CAPACIDADE_DISPONIVEL'			=> $tot->CAPACIDADE_DISPONIVEL						,
//				'UM'							=> $tot->UM											,
//				'CARGA_PROGRAMADA'				=> $tot->CARGA_PROGRAMADA							,
//				'QUANTIDADE_TALAO_PROGRAMADA'	=> $tot->QUANTIDADE_TALAO_PROGRAMADA				,
//				'QUANTIDADE_CARGA_PROGRAMADA'	=> $tot->QUANTIDADE_CARGA_PROGRAMADA				,
//				'QTD_TALAO_UTILIZ_PROGRAMADA'	=> $tot->QTD_TALAO_UTILIZ_PROGRAMADA				,
//				'QTD_CARGA_UTILIZ_PROGRAMADA'	=> $tot->QTD_CARGA_UTILIZ_PROGRAMADA				,
//				'CARGA_PENDENTE'				=> $tot->CARGA_PENDENTE								,
//				'QUANTIDADE_TALAO_PENDENTE'		=> $tot->QUANTIDADE_TALAO_PENDENTE					,
//				'QUANTIDADE_CARGA_PENDENTE'		=> $tot->QUANTIDADE_CARGA_PENDENTE					,
//				'CARGA_UTILIZADA'				=> $tot_produzido[$i]->CARGA_UTILIZADA				,
//				'QUANTIDADE_TALAO_UTILIZADA'	=> $tot_produzido[$i]->QUANTIDADE_TALAO_UTILIZADA	,
//				'QUANTIDADE_CARGA_UTILIZADA'	=> $tot_produzido[$i]->QUANTIDADE_CARGA_UTILIZADA	,
//				'CARGA_PROG_UTILIZ'				=> $tot_produzido[$i]->CARGA_PROG_UTILIZ			,
//				'QUANTIDADE_CARGA_PROG_UTILIZ'	=> $tot_produzido[$i]->QUANTIDADE_CARGA_PROG_UTILIZ	,
//				'QUANTIDADE_PARES_UTILIZADA'	=> $tot_par_remessa[$i]->QUANTIDADE_PARES_UTILIZADA	,
//				'QUANTIDADE_PARES_PENDENTE'		=> $tot_par_remessa[$i]->QUANTIDADE_PARES_PENDENTE	,
//				'QUANTIDADE_PARES_PROGRAMADA'	=> $tot_par_remessa[$i]->QUANTIDADE_PARES_PROGRAMADA,
//				'QUANTIDADE_PARES_UTILIZADA'	=> $tot_par_producao[$i]->QUANTIDADE_PARES_UTILIZADA,
//				'PERC_CARGA_PROGRAMADA'			=> $perc_carga_programada							,
//				'PERC_CARGA_UTILIZADA'			=> $perc_carga_utilizada							,
...
//		}
        ...
        if ( !empty($request->retorno) && in_array('VIEW', $request->retorno) ) {
            ...
            $ret = [
				'VIEW' =>	view('ppcp._22020.index.totalizador-diario',[
								'menu'					=> $this->menu,
