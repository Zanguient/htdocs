        ...
        @if ($popup == 1)
            ...
            @endif
                <button type="button" class="btn btn-success popup-show-plano-acao-alterar" data-hotkey="f10" data-loading-text="{{ Lang::get('master.gravando') }}"><span class="glyphicon glyphicon-ok"></span> {{ Lang::get('master.gravar') }}</button>
                <button type="button" id="{{ $imputs['id']}}" class="btn btn-danger popup-show-plano-acao-item " data-hotkey="f11" data-loading-text="{{ Lang::get('master.cancelar') }}"><span class="glyphicon glyphicon-ban-circle"></span> {{ Lang::get('master.cancelar') }}</button>
            </ul>
        ...
        <fieldset class="planoacao">
            ...
            <div class="textarea-grupo" >
                <textarea name="class-p-a-oque" class="form-control obs class-p-a-oque" rows="5" cols="100" required>{{ $tabela[0]->OBSERVACAO }}</textarea>
            </div>
            ...
            <div class="form-group">
				...
				<input type="text" name="class-p-a-quem" class="form-control input-medio class-p-a-quem" value="{{ $tabela[0]->PLANACAO_QUEM}}" required>
			</div>
            
            <div class="form-group">
				<label for="requisicao">Início:</label>
				<input type="date" name="class-p-a-quandod" class="form-control class-p-a-quandod" value="{{ date("Y-m-d",strtotime($tabela[0]->PLANACAO_QUANDO))}}" required>
			</div>
            
            <div class="form-group">
				<label for="requisicao">Fim:</label>
				<input type="date" name="class-p-a-quandod" class="form-control class-p-a-quandot" value="{{ date("Y-m-d",strtotime($tabela[0]->PLANACAO_QUANDOF))}}" required>
			</div>
            ...
            <label>Como:</label>
            <div class="textarea-grupo">
                <textarea name="class-p-a-como" class="form-control obs class-p-a-como" rows="5" cols="100" required>{{ $tabela[0]->PLANACAO_COMO}}</textarea>
            </div>
